...
```clojure
(require '[polyllmith.secrets.interface :as secrets])
...
```clojure
;; In polyllmith.my-base.app namespace
(defn store
...
Integrant resolves `::app/store` → looks for `store` function in `polyllmith.my-base.app` namespace.
...
(ns polyllmith.my-base.config
  (:require [polyllmith.my-base.db :as db]))

{::db/datomic {:storage-dir ".datomic" :system "myapp" :db-name "dev"}}
;; Integrant resolves ::db/datomic → finds `datomic` in polyllmith.my-base.db
...
```clojure
(dev)
(require '[polyllmith.my-base.config :as my-base])
...
```clojure
;; 1. List public vars
(keys (ns-publics 'polyllmith.some.namespace))

;; 2. Check arglists before calling
(:arglists (meta (ns-resolve 'polyllmith.some.namespace 'some-fn)))
```
...
(run-tests 'polyllmith.secrets.interface-test)
```
...
```edn
{:deps {polyllmith/some-component {:local/root "../../components/some-component"}}}
```
...
;; In base config.clj
(ns polyllmith.my-base.config
  (:require [polyllmith.some-component.interface :as some-component]))
...
```edn
:dev {:extra-deps {polyllmith/new-component {:local/root "components/new-component"}
                   polyllmith/new-base {:local/root "bases/new-base"}}}
```
