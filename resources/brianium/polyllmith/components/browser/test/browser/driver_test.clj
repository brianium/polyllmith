(ns {{top/ns}}.{{main/ns}}.browser.driver-test
  "Tier 1 (hermetic) tests for the browser driver. These tests MUST NOT launch
   ...
   Profile-name normalization tests live in `{{top/ns}}.{{main/ns}}.browser.profile-test`
  ...
  (:require [clojure.test :refer [deftest is testing]]
            [{{top/ns}}.{{main/ns}}.browser.driver :as driver]))
...
(deftest close!-on-map-with-nil-context-returns-ok
  (testing "session map missing the driver-internal context key is treated as
            ...
            regression."
    (is (= :ok (driver/close! {})))
    (is (= :ok (driver/close! {:{{top/ns}}.{{main/ns}}.browser.driver/context nil})))
    (is (= :ok (driver/close! {:profile "x"
                               :headed? false
                               :mode :read-only
                               :{{top/ns}}.{{main/ns}}.browser.driver/context nil})))))
...
(deftest close!-on-attached-stub-runs-disconnect-branch
  (testing "stub session with ::cdp-attached? true + ::context non-nil
    ...
    (let [stub {:profile "cdp:stub"
                :{{top/ns}}.{{main/ns}}.browser.driver/context :sentinel-not-a-real-ctx
                :{{top/ns}}.{{main/ns}}.browser.driver/executor
                (java.util.concurrent.Executors/newSingleThreadExecutor)
                :{{top/ns}}.{{main/ns}}.browser.driver/closed? (atom false)
                :{{top/ns}}.{{main/ns}}.browser.driver/cdp-attached? true
                :{{top/ns}}.{{main/ns}}.browser.driver/browser nil
                :{{top/ns}}.{{main/ns}}.browser.driver/playwright nil
                :{{top/ns}}.{{main/ns}}.browser.driver/page nil}]
      (is (= :ok (driver/close! stub)))
      (is (true? @(:{{top/ns}}.{{main/ns}}.browser.driver/closed? stub))))))
