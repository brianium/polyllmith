(ns {{top/ns}}.{{main/ns}}.browser.driver
  ...
  (:require [{{top/ns}}.{{main/ns}}.browser.profile :as profile]
...
;;
;; Executes JavaScript in the attached page context and returns a
;; JSON-serializable result. Used for framework-reactivity bypasses (Vue/React native-setter
...
;; `{{top/ns}}.{{main/ns}}.browser.interface` enforces `:read-only` rejection before
...
(defn type-into!
  "Fill the element matching `selector` with `text` on the session's current
   ...
   `:{{top/ns}}.{{main/ns}}.browser.interface/type` for agent-facing surface.
  ...
  (let [branches (select-keys condition [:selector :url-pattern :load-state :timeout-ms])
    ...
    (cond
      ...
      (let [p (:url-pattern condition)]
        ...
        (when (> (count p) 250)
          ;; Parallel to {{top/ns}}.{{main/ns}}.browser.interface/url-pattern-max-length —
