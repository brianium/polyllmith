(ns {{top/ns}}.{{main/ns}}.browser.core
  "Brick entry point. Delegates to {{top/ns}}.{{main/ns}}.browser.driver for raw Playwright
   interop; this namespace re-exports the public surface as plain pass-through
   fns, matching the components/solana-rpc shape."
  (:require [{{top/ns}}.{{main/ns}}.browser.driver :as driver]))
...
(defn type-into!
  "Fill the element matching `selector` with `text`. Routes to Playwright's
   ...
   `:{{top/ns}}.{{main/ns}}.browser.interface/type`."
