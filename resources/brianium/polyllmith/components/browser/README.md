...
All public functions live on `{{top/ns}}.{{main/ns}}.browser.interface`. Sessions are opaque — depend only on the public keys.
...
- `:profile` is required and must be a string or keyword; rejects blank names, absolute paths (`/etc/passwd`, `\Windows\...`), embedded path separators (`foo/bar`, `foo\bar`), and `..` traversal. Normalization lives in `{{top/ns}}.{{main/ns}}.browser.profile/profile-key`.
